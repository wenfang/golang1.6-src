...
const (
	...
	bitMarkedAll  = bitMarked | bitMarked<<heapBitsShift | bitMarked<<(2*heapBitsShift) | bitMarked<<(3*heapBitsShift)     // 所有的Marked位的标识
	bitPointerAll = bitPointer | bitPointer<<heapBitsShift | bitPointer<<(2*heapBitsShift) | bitPointer<<(3*heapBitsShift) // 所有的指针位的标识
)

// addb returns the byte pointer p+n.
//go:nowritebarrier
func addb(p *byte, n uintptr) *byte { // 返回p+n位置的字节
...
//go:nowritebarrier
func subtractb(p *byte, n uintptr) *byte { // 返回p-n位置的字节
	// Note: wrote out full expression instead of calling add(p, -n)
	// to reduce the number of temporaries generated by the
	// compiler for this trivial expression during inlining.
	return (*byte)(unsafe.Pointer(uintptr(unsafe.Pointer(p)) - n))
...
//go:nowritebarrier
func add1(p *byte) *byte { // 返回p+1位置的字节
...
//go:nosplit
func subtract1(p *byte) *byte { // 返回p-1位置的字节
...
func (h *mheap) mapBits(arena_used uintptr) {
	...
	const bitmapChunk = 8192 // 位图chunk大小8K
...
func heapBitsForObject(p, refBase, refOff uintptr) (base uintptr, hbits heapBits, s *mspan) {
	...
	if s == nil || pageID(k) < s.start || p >= s.limit || s.state != mSpanInUse { // 如果对应页面为空
	...
	hbits = heapBitsForAddr(base) // 取出基地址对应的bits位
	return
}

// prefetch the bits.
func (h heapBits) prefetch() { // 预取该bit
...
// nosplit because it is used during write barriers and must not be preempted.
//go:nosplit
func (h heapBits) next() heapBits { // 生成下一个bit位的heapBits结构
...
func (h heapBits) forward(n uintptr) heapBits { // 获取前n个word的对应的heapBits
...
func (h heapBits) bits() uint32 { // 取出来对应的位数据
